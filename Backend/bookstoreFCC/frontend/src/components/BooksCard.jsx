        ...
        <div className='grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 '>
            {books.map((item)=>(
                ...
                >
                    ...
                    <h4 className='mt-8 text-gray-500'><b>ID : </b> {item._id}</h4>
                    <div className='flex justify-start items-center gap-x-2'>
                        <PiBookOpenTextLight className='text-red-300 text-2xl' />
                        <h2 className='my-1'><b>Title : </b> {item.title}</h2>
                    </div>
                    <div className='flex justify-start items-center gap-x-2'>
                        <BiUserCircle className='text-red-300 text-2xl' />
                        <h2 className='my-1'> <b>Author : </b>{item.author}</h2>
                    </div>
                    <div className='flex justify-between items-center gap-x-2 mt-3 p-4'>
                        <Link to={`/books/read/${item._id}`}>
